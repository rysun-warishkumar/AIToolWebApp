...
import { getPricingLabel, truncate } from '../../utils/helpers'
...
export const PromptCard = ({ prompt }) => {
  ...
  return (
    <>
      <div onClick={handleCardClick} className="card hover-lift p-5 group cursor-pointer">
        <div className="flex items-start justify-between mb-4">
          <h3 className="font-semibold text-lg flex-1 text-gray-900 dark:text-white group-hover:text-primary-600 transition-colors">
            {prompt.title}
          </h3>
        </div>
        <p className="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2">
          {truncate(prompt.preview_text || prompt.content, 100)}
        </p>
        <div className="flex flex-wrap gap-2 mb-4">
          <span className="badge-secondary text-xs">{prompt.category_name}</span>
          <span className="badge text-xs">{prompt.complexity}</span>
        </div>
